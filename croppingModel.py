...
class resnet50_base(nn.Module):
    ...
    def __init__(self, loadweights=True, downsample=4):
        ...
        self.feature3 = nn.Sequential(resnet50.conv1, resnet50.bn1, resnet50.relu, resnet50.maxpool, resnet50.layer1,
                                      resnet50.layer2)
...
class crop_model_single_scale(nn.Module):
    ...
    def __init__(self, alignsize=8, reddim=8, loadweight=True, model=None, downsample=4, no_rod=False):
        ...
        self._forward_roi_rod = self._forward_roi if no_rod else self._forward_roid
        hidden_dim = reddim if no_rod else reddim * 2
        self.FC_layers = fc_layers(hidden_dim, alignsize)

    def _forward_roid(self, red_feat, boxes):
        RoI_feat = self.RoIAlign(red_feat, boxes)
        RoD_feat = self.RoDAlign(red_feat, boxes)
        final_feat = torch.cat((RoI_feat, RoD_feat), 1)
        return final_feat

    def _forward_roi(self, red_feat, boxes):
        RoI_feat = self.RoIAlign(red_feat, boxes)
        return RoI_feat
    ...
    def forward(self, im_data, boxes):
        ...
        final_feat = self._forward_roi_rod(red_feat, boxes)
...
class crop_model_multi_scale_shared(nn.Module):
    ...
    def __init__(self, alignsize=8, reddim=32, loadweight=True, model=None, downsample=4, no_rod=False):
...
def build_crop_model(scale='single', alignsize=8, reddim=32, loadweight=True, model=None, downsample=4,
                     no_rod=False):
    if scale == 'single':
        return crop_model_single_scale(alignsize, reddim, loadweight, model, downsample, no_rod)
    elif scale == 'multi':
        return crop_model_multi_scale_shared(alignsize, reddim, loadweight, model, downsample, no_rod)
